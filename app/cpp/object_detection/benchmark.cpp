...
void help(char *argv);
void run_object_detection(ObjectDetection* object_detection, std::string image_path);
...
int main (int argc, char **argv)
{
  ...
  while (1) {
    static struct option long_options[] =
      {
        ...
        {"save_inference_result", no_argument, 0, 'r'},
    ...
    } else if (mode == "model_size" or mode == "all") {
      long model_size = object_detection.get_model_size();
      std::cout<<"Model size: "<<(float)model_size /1024.0/ 1024.0<<" Mb"<<std::endl;
    } else if (mode == "fps" or mode == "all") {
      std::cout<<"fps"<<std::endl;
      // run_object_detection(&object_detection, image_dir_path, max_example_num);
    } else if (mode == "accuracy" or mode == "all") {
